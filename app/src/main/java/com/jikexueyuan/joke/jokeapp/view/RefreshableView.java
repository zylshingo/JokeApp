package com.jikexueyuan.joke.jokeapp.view;
...
//     * STATUS_REFRESHING 和 STATUS_REFRESH_FINISHED
    ...
    /**
     * 进行关键性初始化操作，将下拉头向上偏移进行隐藏，给ListView注册touch事件。
     *
     * @param changed
     * @param l
     * @param t
    ...
    /**
     * 当ListView被触摸时候调用，其中处理了各种下拉刷新的具体逻辑。
     *
     * @param view
     * @param motionEvent
     * @return
            ...
            switch (motionEvent.getAction()) {
                ...
                default:
                    if (currentStatus == STATUS_RELEASE_TO_REFRESH) {
                        //松手时候如果是释放立即刷新状态，就去调用正在刷新的任务
                        new RefreshingTask().execute();
                    } else if (currentStatus == STATUS_PULL_TO_REFRESH) {
                        //松手时候如果是下拉状态，就去调用隐藏下拉头的任务
    ...
    private void setIsAbleToPull(MotionEvent event) {
        ...
        if (firstChild != null) {
            ...
            if (firstVisiblePos == 0 && firstChild.getTop() == 0) {
                ...
                //如果首个元素的上边缘距离父布局值为0，就说明ListView滚动到了最顶部，此时应该允许下拉刷新
                ableToPull = true;
            } else {
    ...
    /**
     * 隐藏下拉头的任务，当未进行下拉刷新或者下拉刷新完成后，此任务会将下拉头重新隐藏。
     */
     ...
     * @param id  为了防止不同界面的下拉刷新再上次更新时间上互相有冲突，请不同界面在注册下拉刷新监听器时候
     *            一定要传入不同的id。
     */
